...
class OverlayView(context: Context?, attrs: AttributeSet?) :
    ...
    private val REQUIRED_STABLE_FRAMES = 90  // ~3 seconds at 30fps (strict stability requirement) - now adaptive!
    private var isCapturing = false

    // Phase 1: Adaptive Stability - Track current finger width for dynamic frame requirements
    private var currentFingerWidth: Float = 0f
    ...
    private fun checkStability(): Boolean {
        ...
        val stabilityThreshold = 0.006f  // Increased from 0.003 to be more forgiving
        val isStable = averageDistance < stabilityThreshold

        Log.d(TAG, "[STABILITY] movement=$averageDistance, isStable=$isStable (threshold=$stabilityThreshold)")
        return isStable
    }
    ...
    override fun draw(canvas: Canvas) {
        ...
        results?.let { handLandmarkerResult ->
            ...
            for ((handIndex, landmark) in handLandmarkerResult.landmarks().withIndex()) {
                // OLD CAPTURE LOGIC REMOVED - Now using adaptive stability logic in setResults()
                // The new logic properly checks: quality + stability + position status before capturing
    ...
    private fun areFingersSpread(landmarks: List<NormalizedLandmark>): Boolean {
        ...
        // If average spacing > threshold, fingers are spread
        // Increased from 60 to 90 to be less strict - fingers can be slightly apart
        val threshold = 90f
        Log.d(TAG, "[FINGER SPACING] spacing1=$spacing1, spacing2=$spacing2, spacing3=$spacing3, avg=$avgSpacing (threshold: $threshold)")
    ...
    private fun checkCameraDistance(landmarks: List<NormalizedLandmark>): HandPositionStatus? {
        ...
        if (fingerWidths.isEmpty()) {
            Log.d(TAG, "[FINGER WIDTHS] No valid finger measurements - keeping previous width: $currentFingerWidth")
    ...
    private fun drawCapturingIndicator(canvas: Canvas) {
        if (stableFrameCount > 0 && !isCapturing) {
            // Phase 1: Use adaptive frame requirement for progress calculation
            val requiredFrames = getRequiredStableFrames(currentFingerWidth)
            val progress = stableFrameCount.toFloat() / requiredFrames
            val text = "Hold steady... ${(progress * 100).toInt()}% ($stableFrameCount/$requiredFrames)"
